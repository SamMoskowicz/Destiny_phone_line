...
- **Difficult or current voice turns:** the Realtime model calls a read-only `answer_complex_question` tool backed by `gpt-5.6-sol`, low reasoning, Fast mode, and a 30-second server timeout. That Responses API request can automatically use OpenAI's hosted web search before the Realtime model speaks the answer.
