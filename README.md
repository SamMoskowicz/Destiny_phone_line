...
- **Difficult or current voice turns:** the Realtime model calls a read-only `answer_complex_question` tool backed by `gpt-5.6-sol`, low reasoning, Fast mode, and a 30-second server timeout. That Responses API request can automatically use OpenAI's hosted web search before the Realtime model speaks the answer. When OpenAI actually starts a search, searches that take longer than 650 ms play a quiet two-pulse status cue, with a 1.5-second gap before it repeats.
...
Web search uses the Responses API's hosted `web_search` tool with a low search-context size and at most twenty tool calls per answer. Explicit lookups and clearly time-sensitive questions force the search tool and fail closed if no search call is returned; other potentially changing or uncertain questions let the model choose it automatically. The model skips it for ordinary conversation and stable questions it can answer confidently. Search is enabled by default and uses the existing `OPENAI_API_KEY`; no separate search provider or browser key is needed. Set `OPENAI_WEB_SEARCH_ENABLED=false` to turn it off. Hosted searches add OpenAI tool-call cost and may take longer than answers from model knowledge alone; see the [OpenAI web search guide](https://developers.openai.com/api/docs/guides/tools-web-search).

The voice search cue starts from the Responses API's actual `response.web_search_call.in_progress` event, including when the model chooses search automatically. It is an original sound synthesized directly into Twilio's 8 kHz PCMU format when the service starts. It uses no song or bundled recording, calls no audio-generation or TTS API, and is paced in 20 ms frames so it stays interruptible. It stops before the answer; a Twilio playback marker also lets caller speech clear any final cue audio still buffered after the search finishes. `AI_VOICE_SEARCH_CUE_ENABLED=false` disables it; the delay and quiet repeat interval can be tuned with `AI_VOICE_SEARCH_CUE_DELAY_MS` and `AI_VOICE_SEARCH_CUE_GAP_MS`.
...
The timeouts are upper bounds, not a hard end-to-end guarantee: carrier delay, Twilio, OpenAI load, web search, and the length of the answer also affect timing. The Realtime silence timer is paused while a deep answer is running and restored afterward, and caller speech still cancels an obsolete lookup. Easy spoken turns avoid the second model call and should be the quickest. Fast mode is deliberately enabled because the requested priority is intelligence with bounded latency; set `OPENAI_FAST_MODE=false` to reduce API cost at the expense of latency.
...
- OPENAI_TEXT_TIMEOUT_MS / OPENAI_DEEP_VOICE_TIMEOUT_MS: default to 12000 / 30000; deep voice accepts 500-45000 milliseconds so longer search-backed answers can finish without leaving a call unbounded
